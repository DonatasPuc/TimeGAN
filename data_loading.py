...
def inverse_MinMaxScaler(norm_data, original_data):
    """
    Inverse Min Max normalizer.

    Args:
      - norm_data: Normalized data
      - original_data: Original data for scaling reference

    Returns:
      - data: Data transformed back to original scale
    """
    min_val = np.min(original_data, 0)
    max_val = np.max(original_data, 0)
...
def reconstruct_data(generated_sequences, original_data, seq_len):
  """
  ...
  Args:
    ...
    - original_data: The original data used for finding min and max values for scaling.
  ...
  reconstructed_sequences = [inverse_MinMaxScaler(sequence, original_data) for sequence in generated_sequences]

  # Extract the first value from each sequence (except the last sequence)
  reconstructed_data = [sequence[0] for sequence in reconstructed_sequences[:-1]]

  # Add the entire last sequence
  reconstructed_data.extend(reconstructed_sequences[-1])

  # Flip the data back to its original order
